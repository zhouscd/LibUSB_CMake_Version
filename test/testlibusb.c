...
// Get a list os USB devices    
    ...
    {    
        ...
        if (desc.iSerialNumber) {
			e = libusb_get_string_descriptor_ascii(handle, desc.iSerialNumber, str2, sizeof(str2));
		    if(e < 0)    
            {    
                libusb_free_device_list(devs,1);    
                libusb_close(handle);    
        ...
        if(desc.idVendor == 0x734d && desc.idProduct == 0x0100)    
        {    
            found = 1;    
            break;    
        }   
    ...
    while (1)
    {
        ...
        {    
            printf("\nReceived: ");    
            printf("%s",my_string1);    //will read a string from lcp2148
